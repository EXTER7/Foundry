...
public class BaseMetaTileEntity extends BaseTileEntity implements IGregTechTileEntity {
    ...
    public void updateEntity() {
    	...
    	if (hasValidMetaTileEntity()) {
    	    ...
    	    if (isServerSide()) {
    	        ...
    	        try {
	    	        if (getEUCapacity() > 0) {
		    	        if (GregTech_API.sMachineFireExplosions && getRandomNumber(1000) == 0 && getBlockAtSide((byte)getRandomNumber(6)) == Block.fire) {
		        		    doEnergyExplosion();
		    	        }
	...
	public ArrayList<String> getDebugInfo(EntityPlayer aPlayer, int aLogLevel) {
		...
		if (aLogLevel > 0) {
			...
			if (!mCouldNeedMoreEnergy) tList.add("ATTENTION: This Device consumes Energy at a higher Rate than you input. You could insert more to speed up the process.");
		}
	...
	@Override public boolean decreaseStoredEnergyUnits(int aEnergy, boolean aIgnoreTooLessEnergy) {if (!hasValidMetaTileEntity()) return false; if (getUniversalEnergyCapacity() < aEnergy) mNeedsBatteryUpgrade = true; return mCouldNeedMoreEnergy = decreaseStoredMJ(aEnergy, false) || decreaseStoredSteam(aEnergy, false) || decreaseStoredEU(aEnergy, aIgnoreTooLessEnergy) || (aIgnoreTooLessEnergy && (decreaseStoredMJ(aEnergy, true) || decreaseStoredSteam(aEnergy, true)));}
