...
public class GT_Recipe {
	...
	public static ArrayList<GT_Recipe> sExtruderRecipes		= new ArrayList<GT_Recipe>();
	...
	public static Map<Long, Integer> pExtruderRecipes		= new HashMap<Long, Integer>();
	...
	public static void reinit() {
        ...
        for (Map.Entry<ArrayList<GT_Recipe>, Map<Long, Integer>> tEntry : mRecipeMaps.entrySet()) {
        	...
        	for (int i = 0; i < tEntry.getKey().size(); i++) {
        		GT_OreDictUnificator.setStack(tEntry.getKey().get(i).mInput1);
        		GT_OreDictUnificator.setStack(tEntry.getKey().get(i).mInput2);
        		GT_OreDictUnificator.setStack(tEntry.getKey().get(i).mOutput1);
        		GT_OreDictUnificator.setStack(tEntry.getKey().get(i).mOutput2);
        		GT_OreDictUnificator.setStack(tEntry.getKey().get(i).mOutput3);
        		GT_OreDictUnificator.setStack(tEntry.getKey().get(i).mOutput4);
	...
	public GT_Recipe(int aEUt, int aDuration, int aShape, ItemStack aInput1, ItemStack aOutput1) {
		this(aInput1, null, aOutput1, null, null, null, Math.max(aDuration, 1), Math.max(aEUt, 1), aShape);
		checkCellBalance();
		if (mInput1 != null && mOutput1 != null && findEqualExtruderRecipe(mInput1, mInput2) == null) {
			pExtruderRecipes.put(GT_Utility.stacksToLong(mInput1, mInput2), sExtruderRecipes.size());
			sExtruderRecipes.add(this);
		}
	}
	...
	public static GT_Recipe findEqualExtruderRecipe(ItemStack aInput1, ItemStack aInput2) {
		return findEqualRecipe(aInput1, aInput2, true, sExtruderRecipes, pExtruderRecipes);
	}
	...
	public static int findEqualExtruderRecipeIndex(ItemStack aInput1, ItemStack aInput2) {
		return findEqualRecipeIndex(aInput1, aInput2, true, sExtruderRecipes, pExtruderRecipes);
	}
}