					...
					for (int i = 0; i < sShapes1.length; i++) {
						...
						for (ItemStack tCrafted : GT_ModHandler.getVanillyToolRecipeOutputs(tRecipe)) {
							...
							if (aPlate != null && sShapesA[i] != null && sShapesA[i].length > 1) {
								if (GregTech_API.sConfiguration.addAdvConfig(GT_ConfigCategories.Recipes.recipereplacements, aAssotiation.replaceFirst(OrePrefixes.ingot.toString(), "")+"."+sShapesA[i][0], true)) {
