...
public class MetalCasterRecipeHandler extends FoundryRecipeHandler {
	...
	protected List<ItemStack> getExtraItems(ICastingRecipe recipe) {
		if (recipe.GetInputExtra() == null) return Lists.newArrayList();
	...
	public void drawExtras(int recipe) {
		...
		if(currentProgress > 0) drawProgressBar(new ProgressBar(55, 40, 176, 53, 27, 15, 0, GUI_SMELT_TIME), currentProgress);
	...
	public boolean mouseClicked(GuiRecipe gui, int button, int recipe) {
		...
		if (isMouseOver(castingRecipe.getTank().position, gui, recipe)) {
			if (button == 0) return GuiCraftingRecipe.openRecipeGui("liquid", castingRecipe.getTank().fluid);
			if (button == 1) return GuiUsageRecipe.openRecipeGui("liquid", castingRecipe.getTank().fluid);
		}
		...
		if(outputId.equals("item")) {
			for (ICastingRecipe recipe : CastingRecipeManager.instance.GetRecipes()) {
				if (recipe.GetOutput() != null &&
					recipe.GetOutput() instanceof ItemStack &&
					((ItemStack)recipe.GetOutput()).isItemEqual((ItemStack) results[0]))
